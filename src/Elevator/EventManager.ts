...
export function newId(type: string) {
  lastGeneratedId++
  if (type.length === 0) {
    console.log("\n*** EventManager.ts: newId(type: string): type cannot be empty")
...
function getEventNameFromConstructor<T>(ctor: IEventConstructor<T>): string {
  if (!(eventNameSymbol in ctor) || typeof ctor[eventNameSymbol] !== 'string') {
    console.log ("\n*** EventManager ERROR: The EventConstructor is not registered")
...
export class EventManager {
  ...
  addListener<T, X>(
    ...
    if (!eventClass || typeof (eventClass as any) !== 'function') {
      console.log ("\n*** EventManager ERROR: Invalid EventConstructor")
    ...
    for (let i = 0; i < listeners.length; i++) {
      ...
      if (x.listener === listener) {
        console.log ("\n*** EventManager ERROR: The provided listener is already registered")
...
export function EventConstructor(): ClassDecorator {
  ...
  // lastGeneratedId++
  // const eventName = 'EV'+lastGeneratedId.toString(36)
  

  if (takenEventNames.indexOf(eventName) !== -1) {
    console.log (`\n*** EventManager ERROR: The event name ${eventName} is already taken`)
