import { getCurrentPageGames } from './games-directory/games-state'
import { getConfiguredGridDimensions } from './museumConfig'
...
export async function loadSceneConfig(): Promise<SceneConfig> {
  ...
  try {
    // Load scene.json - try both paths
    let sceneData: any
    try {
    ...
    console.error('Failed to load scene config, using defaults:', error)
    
    // Check for manual parcel count override
    const { MUSEUM_CONFIG } = await import('./museumConfig')
